...
const ProductCardItem = ({ product, categorySlug }: { product: ProductVariant & { categorySlug: string }; categorySlug?: string }) => {
  ...
  return (
    <>
      ...
      `}</style>
      <button type="button" className="pm-card" onClick={() => router.push(productRoute)}>
        {product.popular && (
    ...
    <MainLayout>
      ...
      <div className="pm-page">
        ...
        {selectedCategory && (
          <div style={{ background: "rgba(249,244,236,0.98)", borderBottom: `1px solid ${MIST}`, padding: "14px 0" }}>
            <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
              <button onClick={() => router.push("/products")} style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", cursor: "pointer", fontFamily: "system-ui, -apple-system, sans-serif", fontSize: 14, fontWeight: 700, color: TEXT_SEC, padding: 0 }}>
            ...
            categories.length === 0 ? (
              ...
              <div className="pm-category-grid">
                {displayedCategories.map((cat, idx) => (
                  <button key={cat.slug} className={`pm-cat-card pm-fade pm-fade-${Math.min(idx + 1, 4)}`} onClick={() => router.push(`/products/${cat.slug}`)} style={{ width: "100%", padding: 0 }}>
