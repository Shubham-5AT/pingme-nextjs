...
const CartButton: React.FC<Props> = ({ count, onClick, className }) => {
  const [pulse, setPulse] = useState(false);
  // Suppress hydration mismatch: cart count is always 0 on the server,
  // so we only render count-dependent UI after the client has mounted.
  ...
  return (
    ...
    >
      ...
      <svg className="pm-cart-icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M3 3h2l1.68 9.39a2 2 0 001.98 1.61h7.68a2 2 0 001.98-1.61L21 6H6" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
        <circle cx="10" cy="20" r="1" fill="currentColor" />
        <circle cx="18" cy="20" r="1" fill="currentColor" />
      </svg>

      {mounted && displayCount > 0 && (
        <span className="pm-cart-badge" aria-hidden="true">
