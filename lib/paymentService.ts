...
const getPaymentApiBaseUrl = (): string => {
  const base = process.env.NEXT_PUBLIC_PAYMENT_API_BASE_URL;
  ...
  if (typeof window !== "undefined" && window.location) {
    ...
    console.warn(
      "NEXT_PUBLIC_PAYMENT_API_BASE_URL is not configured. Falling back to window.location.origin for payment API requests.",
      fallback,
    );
    return fallback;
  }
...
export const createRazorpayOrder = async (input: {
  ...
  if (!baseUrl) {
    throw new Error("Payment API is not configured. Add NEXT_PUBLIC_PAYMENT_API_BASE_URL to your env.");
  }
...
export const openRazorpayCheckout = async (input: {
  ...
  const keyId = input.keyId || process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID;
  if (!keyId) {
